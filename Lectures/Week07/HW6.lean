...
theorem Problem1 {n : ℕ} (arr : BitonicSortedArrayFun n) :
  ...
  set peak := arr.peak_idx
  have spec := arr.peak_idx_spec
  obtain ⟨ h0, ⟨ h1,h2 ⟩  ⟩ := spec
  simp [IsMaximum, StrictMonoOn, StrictAntiOn] at *
  constructor
  · constructor <;> grind
  · intro peak' isMax; grind
...
theorem gen2 {a b n :ℕ} (od :a ≤ b)(arr : BitonicSortedArrayFun n) :
  IsPeakInRange arr a b ↔ find_idx_peak.aux arr a b = arr.peak_idx := by
  constructor <;>
  {
    unfold IsPeakInRange; simp
    intros
    have ⟨bruh0, ⟨ bruh1,bruh2⟩ ⟩  := arr.peak_idx_spec
    simp_all [StrictMonoOn, StrictAntiOn]
    fun_induction find_idx_peak.aux <;> try grind [find_idx_peak.aux]
  }
...
def Problem3A (a b c d e f : ℕ) : Option ℕ := do
  let ab ← safeDivide (a*b) c
  let de ← safeDivide (d*e) f
  let res ← safeSub ab de
  pure res
...
def Problem3B (a b c d e f : ℕ) : Option ℕ :=
  safeDivide (a*b) c >>= fun ab =>
  safeDivide (d*e) f >>= fun de =>
  safeSub ab de >>= fun res =>
  pure res
...
theorem Problem4B {α : Type} :
    ...
    intro xs; induction' xs with x' xs' IH
    · simp!
    · simpa [bind, pure]
